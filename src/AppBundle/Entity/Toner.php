 ...
 * @ORM\Entity(repositoryClass="AppBundle\Entity\Repository\TonerRepository")
 */
class Toner implements PermittedActionsInterface
{
    /**
     * @ORM\Id
...
//    protected $pageVolume;
     ...
     *
     * @var Machine[]
     * @ORM\ManyToMany(targetEntity="AppBundle\Entity\Machine", mappedBy="toners")
     **/
    protected $machines;
...
//    }
    ...
    {
        return round($this->price / $this->recomendedVolume, 6);
    }
    ...
    public function removeMachine($machine)
    {
        ...
        $machine->removeToner($this);
    }

    /**
     * Devuelve la cantidad de toners para un número de páginas
     * @param $pages
     * @return float|int
     */
     ...
     * @throws \Exception
     */
    public function getColorDescrition()
    {
        if (!$this->getColor()) {

            throw new \Exception('El toner '.$this->getDescription().' no tiene color asignado');
        }
    ...
    public function isBlack() {
        ...
        return $this->getColorDescrition() === TonerColors::BLACK;
    }
